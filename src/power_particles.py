...
class PowerParticleSystem:
    ...
    @staticmethod
    def create_power_change_effect(position: Tuple[float, float], color: Tuple[int, int, int],
                                 is_decrease: bool = False, 
                                 group: Optional[pygame.sprite.Group] = None) -> list:
        ...
        for _ in range(count):
            ...
            particle = PowerParticle(
                position, (vel_x, vel_y), particle_color, 
                size, lifetime, gravity, 0.97, group
            )
