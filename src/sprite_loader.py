...
) -> Optional[Tuple[pygame.Rect, Tuple[int, int]]]:
    """
            ...
            for col in range(cols):
                ...
                if bounds_data:
                    ...
                    if alignment == "right":
                        ...
                        logger.debug(
                            f"Cell({col},{row}): Right-aligning. AnchorRel=({anchor_x_rel},{anchor_y_rel}), TargetX={target_anchor_x}, Blit=({blit_x},{blit_y})"
                        )
                        ...
                        blit_y = (cell_height // 2) - (content_height // 2)
                        logger.debug(
                            f"Cell({col},{row}): Center-aligning. ContentSize=({content_width},{content_height}), Blit=({blit_x},{blit_y})"
                        )
                        ...
                        blit_y = y_min
                        logger.debug(
                            f"Cell({col},{row}): No alignment (None). Bounds=({x_min},{y_min}), Blit=({blit_x},{blit_y})"
                        )
                    ...
                    logger.debug(
                        f"No content found in cell ({col},{row}) of {filename}. Added empty canvas."
                    )
                ...
                if scale_factor != 1.0:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error scaling canvas for cell ({col},{row}) from {filename}: {e}"
                        )
        ...
        if sprites:
            ...
            def draw_grid(surf, sprite_w, sprite_h, grid_cols, grid_rows):
                for i in range(1, grid_cols):
                    pygame.draw.line(
                        surf, (100, 100, 100), (i * sprite_w, 0), (i * sprite_w, surf.get_height())
                    )
                for i in range(1, grid_rows):
                    pygame.draw.line(
                        surf, (100, 100, 100), (0, i * sprite_h), (surf.get_width(), i * sprite_h)
                    )
